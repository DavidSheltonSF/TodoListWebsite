...
export function TodoItem({ todo, onDelete, onToggleCompletion }: Props) {
  if (todo.status === 'pending') {
    return <TodoItemSkeleton todoTitle={todo.title} />;
  }
    ...
    <article className="flex items-center w-full p-[16px] border-gray-soft bg-color-gray-dark rounded-md fade-in-animation">
      <div className="flex flex-1 items-center gap-[16px]">
        <input
          ...
          type="checkbox"
          className="flex items-center justify-center appearance-none text-xl size-[24px] border-gray-soft rounded-md checked:bg-[var(--color-gray)] checked:before:content-['✓']"
        />
